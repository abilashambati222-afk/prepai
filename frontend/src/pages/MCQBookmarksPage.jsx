...
export default function MCQBookmarksPage() {
  ...
  const fetchBookmarks = async () => {
    try {
      const response = await api.get('/mcq/bookmarks');
    ...
    <div className="space-y-8 w-full">
      ...
      <div className="border-b border-brand-border/60 pb-4">
        <h1 className="text-2xl font-black text-white tracking-tight flex items-center gap-2">
          <Bookmark className="w-6.5 h-6.5 text-brand-secondary fill-brand-secondary/10" />
          Saved Question Vault
        </h1>
        <p className="text-xs text-slate-400 leading-relaxed mt-1">
          Review saved placement questions, view solution explanations, and access Gemini AI tutoring.
        </p>
      </div>

      {bookmarks.length === 0 ? (
        <div className="glass-panel p-16 rounded-2xl text-center space-y-4 max-w-lg mx-auto">
          <div className="w-16 h-16 rounded-full bg-white/5 border border-brand-border flex items-center justify-center mx-auto text-slate-500">
            <Bookmark className="w-6.5 h-6.5" />
          </div>
          <div className="space-y-1">
            <h3 className="text-base font-bold text-white">Your Vault is Empty</h3>
            <p className="text-xs text-slate-400 leading-relaxed max-w-sm mx-auto">
              Bookmark questions while taking practice quizzes to save them here for offline study and revision.
            </p>
          </div>
          <div className="pt-2">
            <Link
              to="/mcq-practice"
              className="inline-flex px-5 py-2.5 bg-brand-primary text-white text-xs font-bold rounded-xl hover:brightness-110 transition-all shadow-lg shadow-brand-primary/10"
            >
                  ...
                  {isExpanded && (
                    ...
                    >
                      <div className="p-6 bg-brand-card/30 space-y-6">
                        ...
                        {/* Standard Explanation & AI Helper */}
                        <div className="flex flex-col md:flex-row gap-6 items-stretch">
                          ...
                          <div className="md:w-60 flex flex-col justify-center items-center p-4 border border-dashed border-brand-border rounded-xl bg-white/3 gap-3 text-center">
                            ...
                            <div className="space-y-1">
                              ...
                              <p className="text-[10px] text-slate-400">Get shortcuts and detailed concept explanations.</p>
                            </div>
                            <button
